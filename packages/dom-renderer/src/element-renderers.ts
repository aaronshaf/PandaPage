...
export function renderParagraph(
  ...
  if (
    "verticalAlignment" in paragraph &&
    paragraph.verticalAlignment &&
    paragraph.verticalAlignment !== "auto" &&
    typeof paragraph.verticalAlignment === "string"
  ) {
...
export function renderHeading(
  ...
  // Add vertical alignment (for inline content)
  if (
    "verticalAlignment" in heading &&
    heading.verticalAlignment &&
    heading.verticalAlignment !== "auto" &&
    typeof heading.verticalAlignment === "string"
  ) {
  ...
  table.rows.forEach((row, rowIndex) => {
    ...
    row.cells.forEach((cell) => {
      ...
      if (
        ...
        (cell.paragraphs.length === 1 && cell.paragraphs[0] && cell.paragraphs[0].runs.length === 0)
      ) {
        // Empty cell - don't create empty paragraph
        td.innerHTML = "&nbsp;";
      } else {
        cell.paragraphs.forEach((paragraph) => {
          const p = renderParagraph(paragraph, doc, currentPageNumber, totalPages, {
            inTableCell: true,
          });
          td.appendChild(p);
        });
...
export function renderImage(
  image: Image,
  doc: Document,
  options?: { currentPageNumber?: number; totalPages?: number; lazy?: boolean },
): HTMLElement {
  const img = doc.createElement("img");

  const shouldLazyLoad =
    options?.lazy !== false && options?.currentPageNumber && options?.totalPages;
...
export function renderFootnote(
  ...
  currentPageNumber: number,
  totalPages: number,
  options?: { footnoteMap?: Map<string, any> },
): HTMLElement {
