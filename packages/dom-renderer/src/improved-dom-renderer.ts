...
import { createFootnoteReference, createFootnoteSection, getFootnoteDisplayNumber } from './footnote-utils';
...
export class EnhancedDOMRenderer {
  ...
  private renderFootnoteSection(): HTMLElement | null {
    return createFootnoteSection(this.footnoteMap, this.doc, (element) => this.renderElement(element));
  }
      ...
      case 'heading':
        ...
        if (element.runs) {
          element.runs.forEach(run => {
            h.appendChild(renderEnhancedTextRun(run, this.doc, this.options.enableAdvancedFormatting));
          });
        }
        return h;
      
      case 'table':
        return renderEnhancedTable(
          element,
          this.doc,
          (para) => renderEnhancedParagraph(
            para,
            this.doc,
            {
              enableDropcaps: this.options.enableDropcaps,
