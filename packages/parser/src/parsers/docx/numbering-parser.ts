...
export const parseNumberingXml = (
  xmlContent: string,
): Effect.Effect<NumberingDefinition, DocxParseError> =>
  Effect.gen(function* () {
    ...
    const abstractNumElements = getElementsByTagNameNSFallback(
      doc.documentElement,
      ns,
      "abstractNum",
    );
    ...
    const abstractFormatsTemp = new Map<
      string,
      { levels: Map<number, LevelFormat>; styleLink?: string; numStyleLink?: string }
    >();
    ...
    for (const abstractElement of abstractNumElements) {
      ...
      const styleLink =
        styleLinkElement?.getAttribute("w:val") ||
        styleLinkElement?.getAttribute("val") ||
        undefined;

      const numStyleLinkElement = getElementByTagNameNSFallback(
        abstractElement,
        ns,
        "numStyleLink",
      );
      const numStyleLink =
        numStyleLinkElement?.getAttribute("w:val") ||
        numStyleLinkElement?.getAttribute("val") ||
        undefined;
