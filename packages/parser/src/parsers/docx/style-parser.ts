...
function parseStyle(styleElement: Element, ns: string): DocxStyle | null {
  ...
  const type = styleElement.getAttribute("w:type") as
    | "paragraph"
    | "character"
    | "table"
    | "numbering";
...
function parseParagraphProperties(pPr: Element, ns: string): DocxParagraphProperties {
  ...
  if (textFlowVal) {
    switch (textFlowVal) {
      case "lrV":
        props.textDirection = "lrV";
        break;
      case "tbV":
        props.textDirection = "tbV";
        break;
      case "lrTbV":
        props.textDirection = "lrTbV";
        break;
      case "tbLrV":
        props.textDirection = "tbLrV";
        break;
    }
  }
