...
export function parseDrawingObject(
  ...
  return Effect.gen(function* () {
    ...
    const inlineElement =
      drawingElement.getElementsByTagNameNS(ns.wp, "inline")[0] ||
      drawingElement.getElementsByTagName("wp:inline")[0];
    const anchorElement =
      drawingElement.getElementsByTagNameNS(ns.wp, "anchor")[0] ||
    ...
    const graphicElement =
      drawingContainer.getElementsByTagNameNS(ns.a, "graphic")[0] ||
      drawingContainer.getElementsByTagName("a:graphic")[0];
    if (!graphicElement) return null;

    const graphicData =
      graphicElement.getElementsByTagNameNS(ns.a, "graphicData")[0] ||
      graphicElement.getElementsByTagName("a:graphicData")[0];
    if (!graphicData) return null;

    // Check if it's a picture
    const picElement =
      graphicData.getElementsByTagNameNS(ns.pic, "pic")[0] ||
    ...
    const wpsElement =
      graphicData.getElementsByTagNameNS(ns.wps, "wsp")[0] ||
...
function parseAnchorPosition(anchorElement: Element, ns: any): DrawingPosition {
  ...
  const positionH =
    anchorElement.getElementsByTagNameNS(ns.wp, "positionH")[0] ||
    anchorElement.getElementsByTagName("wp:positionH")[0];
  if (positionH) {
    const relativeFrom = positionH.getAttribute("relativeFrom");
    const posOffset =
      positionH.getElementsByTagNameNS(ns.wp, "posOffset")[0] ||
      positionH.getElementsByTagName("wp:posOffset")[0];
    const align =
      positionH.getElementsByTagNameNS(ns.wp, "align")[0] ||
  ...
  const positionV =
    anchorElement.getElementsByTagNameNS(ns.wp, "positionV")[0] ||
    anchorElement.getElementsByTagName("wp:positionV")[0];
  if (positionV) {
    const relativeFrom = positionV.getAttribute("relativeFrom");
    const posOffset =
      positionV.getElementsByTagNameNS(ns.wp, "posOffset")[0] ||
      positionV.getElementsByTagName("wp:posOffset")[0];
    const align =
      positionV.getElementsByTagNameNS(ns.wp, "align")[0] ||
...
function parseWrapType(anchorElement: Element, ns: any): DrawingObject["wrapType"] {
  if (
    anchorElement.getElementsByTagNameNS(ns.wp, "wrapNone")[0] ||
    anchorElement.getElementsByTagName("wp:wrapNone")[0]
  ) {
    return "none";
  }
  if (
    anchorElement.getElementsByTagNameNS(ns.wp, "wrapSquare")[0] ||
    anchorElement.getElementsByTagName("wp:wrapSquare")[0]
  ) {
    return "square";
  }
  if (
    anchorElement.getElementsByTagNameNS(ns.wp, "wrapTight")[0] ||
    anchorElement.getElementsByTagName("wp:wrapTight")[0]
  ) {
    return "tight";
  }
  if (
    anchorElement.getElementsByTagNameNS(ns.wp, "wrapThrough")[0] ||
    anchorElement.getElementsByTagName("wp:wrapThrough")[0]
  ) {
    return "through";
  }
  if (
    anchorElement.getElementsByTagNameNS(ns.wp, "wrapTopAndBottom")[0] ||
...
function parseWrapSide(anchorElement: Element, ns: any): DrawingObject["wrapSide"] {
  const wrapElements = ["wrapSquare", "wrapTight", "wrapThrough"];

  for (const wrapType of wrapElements) {
    const wrapElement =
      anchorElement.getElementsByTagNameNS(ns.wp, wrapType)[0] ||
...
function parseExtendedImage(
  ...
  return Effect.gen(function* () {
    ...
    const spPr =
      picElement.getElementsByTagNameNS(ns.pic, "spPr")[0] ||
    ...
    const blipFill =
      picElement.getElementsByTagNameNS(ns.pic, "blipFill")[0] ||
...
function parseTransform(spPr: Element, ns: any): DrawingTransform | undefined {
  const xfrm =
    spPr.getElementsByTagNameNS(ns.a, "xfrm")[0] || spPr.getElementsByTagName("a:xfrm")[0];
...
function parseCrop(blipFill: Element, ns: any): ImageCrop | undefined {
  const srcRect =
    blipFill.getElementsByTagNameNS(ns.a, "srcRect")[0] ||
...
function parseEffects(spPr: Element, ns: any): ImageEffects | undefined {
  const effectLst =
    spPr.getElementsByTagNameNS(ns.a, "effectLst")[0] ||
  ...
  // Parse shadow
  const outerShdw =
    effectLst.getElementsByTagNameNS(ns.a, "outerShdw")[0] ||
...
function parseShape(
  ...
  return Effect.gen(function* () {
    ...
    const spPr =
      wpsElement.getElementsByTagNameNS(ns.wps, "spPr")[0] ||
    ...
    if (spPr) {
      const prstGeom =
        spPr.getElementsByTagNameNS(ns.a, "prstGeom")[0] ||
    ...
    if (spPr) {
      ...
      if (xfrm) {
        const ext =
          xfrm.getElementsByTagNameNS(ns.a, "ext")[0] || xfrm.getElementsByTagName("a:ext")[0];
        if (ext) {
    ...
    // Parse text box
    const txbx =
      wpsElement.getElementsByTagNameNS(ns.wps, "txbx")[0] ||
    ...
    if (txbx) {
      const txbxContent =
        txbx.getElementsByTagNameNS(
          ...
          "txbxContent",
        )[0] || txbx.getElementsByTagName("w:txbxContent")[0];
      if (txbxContent) {
...
function parseShapeFill(spPr: Element, ns: any): ShapeFill | undefined {
  ...
  const solidFill =
    spPr.getElementsByTagNameNS(ns.a, "solidFill")[0] ||
  ...
  const gradFill =
    spPr.getElementsByTagNameNS(ns.a, "gradFill")[0] || spPr.getElementsByTagName("a:gradFill")[0];
  if (gradFill) {
...
function parseGradientFill(gradFill: Element, ns: any): ShapeFill {
  ...
  const gsLst =
    gradFill.getElementsByTagNameNS(ns.a, "gsLst")[0] ||
    gradFill.getElementsByTagName("a:gsLst")[0];
  if (gsLst) {
    const gsElements =
      gsLst.getElementsByTagNameNS(ns.a, "gs") || gsLst.getElementsByTagName("a:gs");
