...
import {
  ...
  parseAdvancedFieldRun,
} from "./field-code-parser";
...
describe("Field Code Parser - Complete Coverage", () => {
  ...
  describe("parseAdvancedFieldRun - Run Properties", () => {
    ...
    it("should parse bold from run properties", () => {
      // Test lines 359-360
      const mockRunProps = createMockElement("w:rPr", [createMockElement("w:b", [])]);
    ...
    it("should parse italic from run properties", () => {
      // Test lines 362-364
      const mockRunProps = createMockElement("w:rPr", [createMockElement("w:i", [])]);
    ...
    it("should parse underline from run properties", () => {
      // Test lines 366-367
      const mockRunProps = createMockElement("w:rPr", [createMockElement("w:u", [])]);
  ...
  return {
    ...
    getElementsByTagNameNS: (ns: string, name: string) => {
      return children.filter(
        (child) => child.tagName === `${ns}:${name}` || child.tagName === name,
      );
    },
