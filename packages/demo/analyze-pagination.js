  ...
  try {
    ...
    const testSplitIntoPages = (html, maxHeight) => {
      ...
      const getElementHeight = (element) => {
        ...
        switch (tagName) {
          case 'H1': return Math.max(3, textLines + 1);
          ...
          case 'UL':
          case 'OL': return element.children.length * 1.2 + 0.5;
    ...
    console.log(`Last page contains "prefer).": ${pages54[pages54.length - 1].includes('prefer).') ? '✓' : '✗'}`);
    ...
    console.log(`Last page contains "prefer).": ${pages150[pages150.length - 1].includes('prefer).') ? '✓' : '✗'}`);
    ...
    console.log(`Last page contains "prefer).": ${pages1000[pages1000.length - 1].includes('prefer).') ? '✓' : '✗'}`);
