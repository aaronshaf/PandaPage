...
export function renderHeader(header: any, doc: Document, currentPageNumber: number, totalPages: number): HTMLElement {
  ...
  header.elements.forEach((el: any) => {
    if (el.type === 'paragraph') {
      headerEl.appendChild(renderParagraph(el, doc, currentPageNumber, totalPages, { inTableCell: false }));
...
export function renderHeaderWithPageNumber(header: Header, pageNumber: number, totalPages: number, doc: Document): HTMLElement {
  const headerEl = doc.createElement('header');
...
export function renderFooter(footer: any, doc: Document, currentPageNumber: number, totalPages: number): HTMLElement {
    ...
    if (el.type === 'paragraph') {
      ...
      if (fullText.includes('[Recipient') && fullText.includes('Recovery Plan')) {
        ...
        if (hasTab) {
          ...
          footerEl.appendChild(renderParagraph(el, doc, pageNumber, totalPages, { inTableCell: false }));
        }
      } else {
        footerEl.appendChild(renderParagraph(el, doc, pageNumber, totalPages, { inTableCell: false }));
      }
