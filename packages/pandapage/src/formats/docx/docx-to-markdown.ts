...
import { DocxDocument, DocxParagraph, DocxRun, readDocx, DocxNumbering, DocxLevelFormat } from "./docx-reader";
...
export const convertDocxToMarkdown = (document: DocxDocument): string => {
  ...
  let listCounters = new Map<string, number>(); // Track counters for numbered lists
...
const convertParagraphToMarkdown = (
  ...
  const combinedText = paragraph.runs
...
export const docxToMarkdown = (buffer: ArrayBuffer): Effect.Effect<string, import("./docx-reader").DocxParseError> =>
  Effect.gen(function* () {
