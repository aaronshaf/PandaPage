...
const convertEnhancedDocumentToMarkdown = (
  ...
  Effect.gen(function* () {
    ...
    if (document.metadata && Object.keys(document.metadata).length > 0) {
      ...
      if (document.metadata.created) lines.push(`created: ${document.metadata.created.toISOString()}`);
      if (document.metadata.modified) lines.push(`modified: ${document.metadata.modified.toISOString()}`);
      lines.push("---");
      lines.push("");
    }
    ...
    for (const element of document.elements) {
      if (element.type === "paragraph") {
        ...
        const text = element.runs?.map(run => {
