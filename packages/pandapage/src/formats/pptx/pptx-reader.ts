  ...
  Effect.gen(function* () {
    ...
    try {
      ...
      const { unzipSync, strFromU8 } = yield* Effect.tryPromise({
        try: () => import('fflate'),
        catch: (error) => new PptxParseError({
      ...
      return yield* Effect.fail(new PptxParseError({
...
const parseSlideXml = (xmlContent: string, slideNumber: number): Effect.Effect<PptxSlide, PptxParseError> =>
  Effect.gen(function* () {
    ...
    while ((shapeMatch = shapeRegex.exec(xmlContent)) !== null) {
      ...
      while ((paragraphMatch = paragraphRegex.exec(shapeContent)) !== null) {
        ...
        const hasBullet = /<a:buChar/.test(paragraphContent) || 
                         /<a:buNone/.test(paragraphContent) === false;
...
const parsePresentationProps = (xmlContent: string): Effect.Effect<PptxDocument["metadata"], PptxParseError> =>
  Effect.gen(function* () {
