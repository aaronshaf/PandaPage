...
describe('List Rendering Tests', () => {
  ...
  test('should parse 001.docx and detect list items', async () => {
    ...
    // Find list items
    const listItems = parsedDocument.elements.filter(el => 
      el.type === 'paragraph' && (el as any).listInfo
    );
    ...
    console.log(`Found ${bulletLists.length} bullet list items and ${numberLists.length} number list items`);
  ...
  test('should group consecutive list items correctly', async () => {
    ...
    console.log(`Found ${ulMatches!.length} unordered lists and ${olMatches!.length} ordered lists`);
  });
