...
export const parseHyperlinkRelationships = (
  ...
  Effect.gen(function* () {
    ...
    const doc = yield* parseXmlString(relsXml).pipe(
      Effect.mapError(
        (error) => new DocxParseError(`Failed to parse relationships XML: ${error.message}`),
      ),
    );
...
export const parseHyperlink = (
  ...
  Effect.gen(function* () {
    ...
    const doc = yield* parseXmlString(xmlContent).pipe(
      Effect.mapError(
        (error) => new DocxParseError(`Failed to parse hyperlink XML: ${error.message}`),
      ),
    );
    ...
    const rId =
      hyperlinkElement.getAttributeNS(relNamespaceURI, "id") ||
    ...
    const anchor =
      hyperlinkElement.getAttributeNS(wordNamespaceURI, "anchor") ||
