    ...
    for (const pElement of finalParagraphs) {
      ...
      if (pPrElements.length > 0) {
        ...
        if (numPrElements && numPrElements.length > 0) {
          ...
          numId = numIdElements && numIdElements.length > 0 ? numIdElements[0]?.getAttribute('w:val') || undefined : undefined;
          ilvl = ilvlElements && ilvlElements.length > 0 ? parseInt(ilvlElements[0]?.getAttribute('w:val') || '0', 10) : undefined;
        }
      }
...
export const parseNumberingXmlWithDom = (xmlContent: string): Effect.Effect<any, DocxParseError> =>
  Effect.gen(function* () {
    ...
    for (const numElement of numElements) {
      ...
      let abstractNumIdElement = numElement.getElementsByTagNameNS(wordNamespaceURI, 'abstractNumId')[0];
