    ...
    parsed.elements.forEach((element, index) => {
      switch (element.type) {
        case "heading":
          headingsFound++;
          console.log(
            `Heading ${element.level}: "${element.runs
              .map((r) => r.text)
              .join("")
              .trim()}"`,
          );
        ...
        case "table":
          tablesFound++;
          console.log(
            `Table with ${element.rows.length} rows, ${element.rows[0]?.cells.length || 0} columns`,
          );
          break;
        ...
        case "paragraph":
          ...
          if (text.length > 0 && (element.style || element.runs.some((r) => r.bold || r.italic))) {
            console.log(
              `Formatted paragraph: "${text.substring(0, 100)}${text.length > 100 ? "..." : ""}" (style: ${element.style || "none"})`,
            );
          }
          break;
        ...
        case "footnote":
          const footnoteText = element.runs
            ? element.runs
                .map((r) => r.text)
                .join("")
                .trim()
      ...
      if (element.type === "paragraph") {
        ...
        if (text.length > 0) {
          ...
          element.runs.forEach((run, runIndex) => {
            if (run.bold || run.italic || run.underline || run.color || run.fontSize) {
              console.log(
                `    Run ${runIndex + 1}: "${run.text.substring(0, 40)}${run.text.length > 40 ? "..." : ""}" - ${JSON.stringify(
                  {
                    bold: run.bold,
                    italic: run.italic,
