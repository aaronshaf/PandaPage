  ...
  try {
    ...
    if (stylesFile) {
      ...
      for (let i = 0; i < styles.length; i++) {
        ...
        console.log(
          `  ${i + 1}. Type: ${styleType}, ID: ${styleId}, Name: "${displayName}", Default: ${isDefault}, BasedOn: ${basedOn}`,
        );
        ...
        if (
          ...
          if (rPr) {
            ...
            console.log(
              `    Bold: ${!!bold}, Size: ${size?.getAttribute("w:val") || "default"}, Color: ${color?.getAttribute("w:val") || "auto"}`,
            );
          }
        }
      }
    ...
    if (documentFile) {
      ...
      for (let i = 0; i < Math.min(15, paragraphs.length); i++) {
        ...
        if (text) {
          console.log(
            `Paragraph ${i + 1}: Style="${styleId}", Text="${text.substring(0, 60)}${text.length > 60 ? "..." : ""}"`,
          );
        }
      }
    }
