...
import type { DocxElement, DocxTable, DocxParagraph } from "@browser-document-viewer/core";
...
export const DocxRenderer: React.FC<DocxRendererProps> = ({
  ...
  const renderTable = (table: DocxTable, index: number) => {
    ...
    const tableClasses =
      viewMode === "print" ? "table-fixed border-collapse" : "table-auto border-collapse w-full";

    return (
      <table key={index} className={tableClasses} style={tableStyle} data-testid={`table-${index}`}>
        <tbody>
  ...
  const renderParagraph = (paragraph: DocxParagraph, key: string) => {
    ...
    runs.forEach((run: any, index: number) => {
      let textElement: React.ReactElement = (
        <React.Fragment key={`run-${index}`}>{run.text}</React.Fragment>
      );
  ...
  return (
    <div
      className={
        className || (viewMode === "print" ? "print-content" : "prose prose-gray max-w-none")
      }
      style={style}
    >
