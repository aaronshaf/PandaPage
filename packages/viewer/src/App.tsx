...
const App: React.FC = () => {
  ...
  const getDocumentTitle = () => {
    ...
    // Fall back to structured document metadata
    ...
    if (structuredDocument?.elements) {
      for (const element of structuredDocument.elements) {
        if (element.type === "paragraph") {
          ...
          if (
            element.style &&
            (element.style.toLowerCase().includes("heading") ||
              element.style.toLowerCase().includes("title"))
          ) {
            const text = element.runs
              ?.map((run: any) => run.text || "")
              .join("")
  ...
  const updateUrlHash = (docId?: string, mode?: "read" | "print") => {
    const currentDocId =
      docId ||
      (selectedDocument ? selectedDocument.split("/").pop() : sampleDocuments[0]?.id || "");
  ...
  const parseErrorMessage = (error: any): string => {
    ...
    if (error && typeof error === "object") {
      ...
      if (error.message) {
        ...
        if (
          message.includes("Pages support is not yet implemented") ||
    ...
    if (typeof error === "string") {
      if (
        error.includes("Pages support is not yet implemented") ||
    ...
    try {
      ...
      if (fileName.endsWith(".docx")) {
        try {
          ...
          // Keep structured document for backward compatibility
          try {
          ...
          console.error("Failed to use new parser, falling back:", parseError);
