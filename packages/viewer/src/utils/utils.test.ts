...
test("removeFrontmatter should remove YAML frontmatter from markdown", () => {
  const markdown = `---
title: Test Document
author: John Doe
---
...
test("removeFrontmatter should return original markdown if no frontmatter", () => {
...
test("removeFrontmatter should handle malformed frontmatter", () => {
  const markdown = `---
title: Test Document
# Heading 1
This is content.`;
...
test("removeFrontmatter should handle empty frontmatter", () => {
  const markdown = `---
---
...
test("countWords should handle newlines and tabs", () => {
...
test("extractHeadings should extract headings from markdown", () => {
  const markdown = `# Heading 1
Some content
...
test("extractHeadings should handle headings with special characters", () => {
...
test("extractHeadings should ignore invalid headings", () => {
...
test("extractHeadings should handle no headings", () => {
...
test("extractHeadingsFromDocument should extract headings from structured document", () => {
  const document = {
    elements: [
      {
        type: "paragraph",
...
test("extractHeadingsFromDocument should handle empty or malformed document", () => {
...
test("extractHeadingsFromDocument should ignore empty headings", () => {
  const document = {
    elements: [
      {
        type: "heading",
...
test("extractHeadingsFromDocument should handle headings with no runs", () => {
  const document = {
    elements: [
      {
        type: "heading",
...
// getBasePath tests
test("getBasePath should return correct base path in different environments", () => {
...
test("document title extraction should prioritize document metadata title", () => {
...
test("document title extraction should fall back to first heading when no metadata title", () => {
  const metadata = {};
  const headings = [
...
test("document title extraction should fall back to filename when no metadata or headings", () => {
  const metadata = {};
  const headings: any[] = [];
...
test("document title extraction should handle empty values gracefully", () => {
