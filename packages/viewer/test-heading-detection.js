...
async function testHeadingDetection() {
  ...
  try {
    ...
    parsed.elements.forEach((element, index) => {
      if (element.type === "heading") {
        ...
        if (index < 15 && text.length > 0) {
          console.log(
            `Paragraph ${index + 1}: "${text.substring(0, 80)}${text.length > 80 ? "..." : ""}"`,
          );
        }
      }
    });
    ...
    const expectedHeadings = ["TITLE OF THE CHICAGO FORMAT PAPER", "Bibliography"];

    console.log(`\\n=== CHECKING EXPECTED HEADINGS ===`);
    expectedHeadings.forEach((expectedText) => {
      const found = parsed.elements.find(
        (el) =>
          el.type === "heading" &&
          el.runs
            .map((r) => r.text)
      ...
      console.log(
        `"${expectedText}": ${found ? "✓ FOUND as heading level " + found.level : "✗ NOT FOUND"}`,
      );
    });
      ...
      if (element.type === "paragraph") {
        ...
        if (text.length > 0) {
          ...
          if (hasBold || hasLargeFont || isAllCaps || isCenter || (isShort && index < 10)) {
            ...
            console.log(
              `  Bold: ${hasBold}, Large: ${hasLargeFont}, AllCaps: ${isAllCaps}, Center: ${isCenter}, Short: ${isShort}`,
            );
          }
        }
      }
