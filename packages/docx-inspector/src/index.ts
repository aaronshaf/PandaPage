...
export class DocxInspector {
  ...
  private determineFileType(path: string, content: string | ArrayBuffer): 'xml' | 'binary' | 'unknown' {
  ...
  private isEmbeddedObject(path: string): boolean {
    return path.includes('word/embeddings/') || 
  ...
  getNumberingXml(result: DocxInspectionResult): string | null {
    ...
    return numberingFile && typeof numberingFile.content === 'string' ? numberingFile.content : null;
  }
  ...
  getRelationshipsForPart(result: DocxInspectionResult, partPath: string): DocxRelationship[] {
    ...
    return result.relationships.filter(rel => 
      result.files.some(f => f.name === relsPath)
    );
  }
